// 我独自生活修改器 v2.0 - WoduziCheat_v2.m
// 基于NSUserDefaults直接修改，适配真实存档结构
...
static NSString* getLogPath(void) {
    ...
    return [documentsPath stringByAppendingPathComponent:@"woduzi_cheat_v2.log"];
}
...
static BOOL modifyGameData(NSInteger money, NSInteger stamina, NSInteger health, NSInteger mood, NSInteger experience) {
    ...
    for (NSString *key in moneyKeys) {
        ...
        if (value) {
            writeLog([NSString stringWithFormat:@"  %@: %@", key, value]);
        }
    }
    ...
    if (mood > 0) {
        for (NSString *key in moodKeys) {
            id value = [defaults objectForKey:key];
            if (value) {
                [defaults setInteger:mood forKey:key];
    ...
    if (experience > 0) {
        for (NSString *key in experienceKeys) {
            id value = [defaults objectForKey:key];
            if (value) {
                [defaults setInteger:experience forKey:key];
    ...
    if (success && modifiedCount > 0) {
        writeLog([NSString stringWithFormat:@"✅ 成功修改 %d 个字段", modifiedCount]);
        writeLog(@"🎉 修改完成！");
    } else if (modifiedCount == 0) {
        writeLog(@"⚠️ 未找到任何可修改的字段");
        success = NO;
    } else {
        writeLog(@"❌ 保存失败");
        success = NO;
